...
function setup(){
	largura=window.innerHeight/8.8;
	altura=window.innerWidth/5.08;
	canvas=createCanvas(1200, 600)
	canvas.position(largura,altura)
	canvas.class("jogo");
...
function setPlay(){
		if(v>0){
		background(0, 153, 255,255)
		if(keyIsDown(193)){ //setas direita e esquerda movimentam o personagem
			posX+=8
		}
	...
	if(bonusGain == true){
		...
		if(keyIsDown(186)){ //seta de cima para pular 
			pulo=1
		}
		if(pulo==1){
			posY-=20
